...
Tree.prototype = {
  ...
  addChild: function() {
    var self = this
    if (this.iOpen.classList.contains("img-close")) {
      this.toggleChilds()
    } //如果父元素close，则打开它
    ...
    if (patt.test(labels[i].firstChild.data)) {
      ...
      function openToRoot(searchTree) {
        if (searchTree.parent) {
          if (searchTree.parent.iOpen.classList.contains("img-close")) {
            searchTree.parent.toggleChilds()
          }
